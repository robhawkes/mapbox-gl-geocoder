...
MapboxGeocoder.prototype = {
  ...
  _geocode: function(searchInput) {
    return new Promise(function (resolve, reject) {
      ...
      if (this.options.remoteGeocoder) {
        requests.push(this.options.remoteGeocoder(searchInput));
      }

      Promise.all(requests).then(function (responses) {
        this._loadingEl.style.display = 'none';

        var res = responses[0].entity;

        if (responses[1] && responses[1].length > 0) {
          // supplement Mapbox Geocoding API results with remote results
          res.features = res.features ? responses[1].concat(res.features) : responses[1];
        }
